...
const ORIGINAL_LOGIC_FIELDS = new Set();
...
export default function Ass2MarkingPage({ zid }) {
  ...
  useEffect(() => {
    ...
    async function loadMarkingRecord() {
      ...
      try {
        ...
        const schemaBlob = payload.score?.submission?.marking_schema_blob ?? payload.score?.marking_schema_blob ?? [];
        const markingBlob = payload.score?.submission?.marking_blob ?? payload.score?.marking_blob ?? [];
  ...
  function buildPayload() {
    ...
    const blobByField = getBlobByField(score?.submission?.marking_blob ?? score?.marking_blob ?? []);
